...
\textbf{3.1.3.2 Exported Access Programs}
\begin{itemize}
    \item \texttt{start\_game\_sequence()}
    \item \texttt{finish\_game(winner: String)}
    \item \texttt{apply\_customizations()}
\end{itemize}

\textbf{3.1.4 Semantics}\\
\textbf{3.1.4.1 State Variables}\\
\begin{itemize}
    \item \texttt{games\_played}: Tracks completed games.
...
\begin{itemize}
    \item \texttt{start\_game\_sequence()}\\
    \textbf{Transition:} Initializes the first game in the sequence.\\
    \textbf{Exception:} Throws error if no players are connected.

    \item \texttt{finish\_game(winner: String)}\\
    \textbf{Transition:} Updates win count and triggers customization phase.

    \item \texttt{apply\_customizations()}\\
    \textbf{Transition:} Applies player-selected upgrades to the next game.
\end{itemize}

\textbf{3.1.4.5 Local Functions}
\begin{itemize}
    \item \texttt{update\_scores()}
    \item \texttt{check\_win\_conditions()}
\end{itemize}
...
\textbf{3.3.3 Syntax}\\
\textbf{3.3.3.1 Exported Constants}\\
None.  

\textbf{3.3.3.2 Exported Access Programs}
\begin{itemize}
    \item \texttt{update\_player\_stats(player\_id: String, stats: Dictionary)}
    \item \texttt{apply\_consumable(player\_id: String, consumable: String)}
\end{itemize}

\textbf{3.3.4 Semantics}\\
\textbf{3.3.4.1 State Variables}\\
\begin{itemize}
    \item \texttt{player\_stats}: A dictionary containing player-specific data (health, score, etc.).
\end{itemize}
...
\begin{itemize}
    \item \texttt{update\_player\_stats(player\_id, stats)}\\
    \textbf{Transition:} Updates player state variables based on the input dictionary.

    \item \texttt{apply\_consumable(player\_id, consumable)}\\
    \textbf{Transition:} Applies the effect of a consumable item to the specified player.
\end{itemize}

\textbf{3.3.4.5 Local Functions}
\begin{itemize}
    \item \texttt{track\_modifiers()}
    \item \texttt{reset\_player\_state()}
\end{itemize}
